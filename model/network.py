...
class Net(nn.Module):
    ...
    def forward(self, x, return_cat=False, return_intermediates=False, return_descriptor=False):
        p1, p2, p3, p4 = self.backbone(x)

        # Use the modified DHT_Layer to optionally extract the descriptor vectors.
        if return_descriptor:
            p1 = self.dht_detector1(p1, return_descriptor=True)
            p2 = self.dht_detector2(p2, return_descriptor=True)
            p3 = self.dht_detector3(p3, return_descriptor=True)
            p4 = self.dht_detector4(p4, return_descriptor=True)
        else:
            p1 = self.dht_detector1(p1)
            p2 = self.dht_detector2(p2)
            p3 = self.dht_detector3(p3)
        ...
        if return_intermediates:  # for descriptor comparison, etc.
            return p1, p2, p3, p4

        cat = self.upsample_cat(p1, p2, p3, p4)
        logist = self.last_conv(cat)

        if return_cat:  # for descriptor comparison, etc.
            return logist, cat
        return logist
